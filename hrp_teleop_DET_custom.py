...
#DET_custom entry*****
      ...
      try:
        ...
        while rospy.Time.now() < now + rospy.Duration.from_sec(13):
        ...
        while rospy.Time.now() < now + rospy.Duration.from_sec(16):
          self.command = np.array([-1,0])
          self.update()
          rate.sleep()
          
        while rospy.Time.now() < now + rospy.Duration.from_sec(21):
          self.command = np.array([0,1])
          self.update()
          mode = UInt16()
          mode.data = Mode.MODE_SOUND_CLICK
        ...
        while rospy.Time.now() < now + rospy.Duration.from_sec(26):
          self.command = np.array([0,-1])
          self.update()
          rate.sleep()

        while rospy.Time.now() < now + rospy.Duration.from_sec(28):
          self.command = np.array([1,0])
          self.update()
          mode = UInt16()
          mode.data = Mode.MODE_SOUND_CLICK
        ...
        while rospy.Time.now() < now + rospy.Duration.from_sec(30):
          self.command = np.array([-1,0])
          self.update()
          rate.sleep()
